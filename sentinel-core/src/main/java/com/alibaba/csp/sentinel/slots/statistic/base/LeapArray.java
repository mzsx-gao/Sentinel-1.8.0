...
public abstract class LeapArray<T> {
    ...
    public WindowWrap<T> currentWindow(long timeMillis) {
        ...
        /*
         * 计算当前时间应该落在哪个时间窗格
         * 加入当前两个窗格，第一个：0-500ms,第二个:500-1000ms,例如当前时间是800，800/500结果取整就是1，
         * 1对2取余就是1，落在第2个窗格
         */
        int idx = calculateTimeIdx(timeMillis);
        // 计算当前窗格的开始时间，当前时间戳对每个窗格的时间长度取模，然后用当前时间戳减去这个结果，例如当前1600ms，
        // 每个时间窗格是500ms,则这里计算的结果就是1500ms
        ...
        while (true) {
            ...
            if (old == null) {
            ...
            } else if (windowStart == old.windowStart()) {//落在同一窗格的请求直接返回老窗格，例如800ms和810ms的请求返回同一窗格
                /*
                 *     B0       B1      B2     B3      B4
                 * ||_______|_______|_______|_______|_______||___
                 * 200     400     600     800     1000    1200  timestamp
                ...
                /*
                 * 如果需要加入新的窗格，则覆盖老的格子，例如1600ms时的请求，落在第二窗格，但是开始时间不一样，则删除500-1000ms这个窗格
                 *   (old)
                 *             B0       B1      B2    NULL      B4
            ...
            } else if (windowStart < old.windowStart()) {//发生时钟回拨
                // Should not go through here, as the provided time is already behind.
                // 返回新的窗格，跟上面数组里的窗格没有任何关系，理论上不应该走到这里
